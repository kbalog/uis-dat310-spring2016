...
<head>
    <meta charset="UTF-8">
    <title>Exercise #6a: Saving users to MySQL table</title>
    <style>
        div {
            margin-bottom: 5px;
            padding: 3px;
...
function save_to_mysql($name, $email, $year, $month, $day, $sex) {
    ...
    $stmt = $mysqli->prepare("INSERT INTO users(name, email, date_of_birth, sex) VALUES (?, ?, ?, ?)");
    // bind parameters
    $date_of_birth = $year . "-". $month . "-" . $day;
    $stmt->bind_param('ssss', $name, $email, $date_of_birth, $sex);
    $stmt->execute();
